...
const Poster = ({ player }) => {
  ...
  useEffect(() => {
    ...
    ImageUrl({ player, pathOrUrl: posterImage, width: 200 }).then((imageUrl) =>
  ...
  return (
    <img alt="Video Poster" src={imageUrl} className={PlayerStyles["poster"]} />
  );
};
...
const PlayerUI = ({
  ...
  const onUserAction = ({ action, text }) =>
    setRecentUserAction({ action, text, key: Math.random() });

  useEffect(() => {
    setMounted(true);

    // Observe target portal size
    const disposeResizeObserver = ObserveResize({
      target,
      setSize,
      setOrientation,
      setDimensions,
  ...
  useEffect(() => {
    ...
    try {
      ...
      parameters.clientOptions.client =
        client || parameters.clientOptions.client;
      ...
      const disposePlayerSettingsListener =
        newPlayer.controls.RegisterSettingsListener(() => {
      ...
      const disposeVisibilityObserver = ObserveVisibility({
        player: newPlayer,
      });
      ...
      // Media session
      const disposeMediaSessionObserver = ObserveMediaSession({
        player: newPlayer,
      });
      ...
      // Can play
      const disposeCanPlayListener =
        newPlayer.controls.RegisterVideoEventListener("canplay", () =>
          setCanPlay(true)
        );
      ...
      return () => {
        videoRef &&
          videoRef.current &&
          videoRef.current.removeEventListener("play", setPlaybackStarted);
  ...
  return (
    <div
      ...
      style={{
        backgroundColor:
          parameters.playerOptions.backgroundColor || "transparent",
        "--portal-width": `${
          shouldRotate ? dimensions.height : dimensions.width
        }px`,
        "--portal-height": `${
          shouldRotate ? dimensions.width : dimensions.height
        }px`,
      }}
      className={[
        PlayerStyles["player-container"],
        shouldRotate ? PlayerStyles["player-container--rotated"] : "",
        `__eluvio-player--size-${size}`,
        `__eluvio-player--orientation-${orientation}`,
      ].join(" ")}
    >
      {!showPlayerProfileForm || !playerInitialized ? null : (
        <PlayerProfileForm
          player={player}
          Close={() => player.controls.HidePlayerProfileForm()}
        />
      )}
      <video
        playsInline
        ...
        ].includes(parameters.playerOptions.muted)}
        controls={
          parameters.playerOptions.controls ===
      ...
      {playerInitialized ||
      errorMessage ||
      !parameters.playerOptions.showLoader ? null : (
      ...
      {!player ? null : parameters.playerOptions.ui ===
        EluvioPlayerParameters.ui.WEB ? (
        <WebControls
          player={player}
